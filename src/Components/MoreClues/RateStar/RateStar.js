...
function RateStar({openAddComment, star= 3, number= 0}) {
    ...
    useEffect(() => {
        ...
        for(let i=0; i<5; i++){
            if(i< star)
                exp.push(<img key={i} src="/image/svg/yellowStar.svg" alt=""/>);
    ...
    return (
        ...
        <p className={styles.header}>{t("vote")}</p>
            <div className={styles.boxAvg}>
                ...
                <p className={styles.number}>{`${t("baseOn1")} ${number} ${t("baseOn2")}`}</p>
            </div>
        ...
        <button className={styles.addComment} onClick={openAddComment}>{t("AddComment")}</button>
        </div>
